...
class MLP(nn.Module):
    ...
    def forward(self, x: Tensor, gate_w: Tensor, up_w: Tensor, down_w: Tensor) -> Tensor:
        # 1. Calculate the Gate (routing signal) and apply SiLU
        gate = F.silu(F.linear(x, gate_w.to(x.dtype)))
        
        # 2. Calculate the Value (data payload)
        value = F.linear(x, up_w.to(x.dtype))
        
        # 3. Multiply them together (SwiGLU)
        hidden = gate * value
...
class GPT(nn.Module):
    ...
    def _init_weights(self) -> None:
        ...
        for i in range(n):
            ...
            nn.init.orthogonal_(self.mlp_gate_bank.data[i], gain=1.0)  # MLP gate
