...
class DistributedTokenLoader:
    ...
    def next_batch(self, global_tokens: int, seq_len: int, grad_accum_steps: int) -> tuple[Tensor, Tensor]:
        ...
        if self.pin_buffer is None or self.pin_buffer.numel() != per_rank_span:
            self.pin_buffer = torch.empty(per_rank_span, dtype=torch.int64, pin_memory=True)
            
        chunk = self.stream.take(per_rank_span * self.world_size)
        start = self.rank * per_rank_span
        self.pin_buffer.copy_(chunk[start : start + per_rank_span])
        
        x = self.pin_buffer[:-1].reshape(-1, seq_len)
        y = self.pin_buffer[1:].reshape(-1, seq_len)
