...
class DistributedTokenLoader:
    ...
    def next_batch(self, global_tokens: int, seq_len: int, grad_accum_steps: int):
        ...
        # PCIe payload optimization: strictly transmit in tight 16-bit buffers, natively unpack to 64-bit on target GPU!
        x_16 = x.to(torch.int16)
        y_16 = y.to(torch.int16)
        return x_16.to(self.device, non_blocking=True).to(torch.int64), y_16.to(self.device, non_blocking=True).to(torch.int64), pos_x.to(self.device, non_blocking=True), cu_seqlens.to(self.device, non_blocking=True), max_seqlen
