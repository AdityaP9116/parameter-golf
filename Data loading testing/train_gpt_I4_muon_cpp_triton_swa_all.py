...
class DistributedTokenLoader:
    ...
    def _prefetch_worker(self, global_tokens: int, seq_len: int, grad_accum_steps: int):
        ...
        # 2. Pinned Memory initialization outside loop
        self.pin_buffer = torch.empty(per_rank_span, dtype=torch.int16, pin_memory=True)
        
        while True:
            chunk = self.stream.take(per_rank_span * self.world_size)
            start = self.rank * per_rank_span
            
            # Pinned memory blocking copy on CPU
            self.pin_buffer.copy_(chunk[start : start + per_rank_span])
            
            x_cpu = self.pin_buffer[:-1].reshape(-1, seq_len)
            y_cpu = self.pin_buffer[1:].reshape(-1, seq_len)
            
            with torch.cuda.stream(self.cuda_stream):
                # 4. Late GPU Casting (transmit 16-bit to device asynchronously, cast exactly on GPU)
                x_gpu = x_cpu.to(self.device, non_blocking=True).to(torch.int64)
                y_gpu = y_cpu.to(self.device, non_blocking=True).to(torch.int64)
