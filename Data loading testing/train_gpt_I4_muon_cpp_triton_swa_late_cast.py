...
class DistributedTokenLoader:
    ...
    def next_batch(self, global_tokens: int, seq_len: int, grad_accum_steps: int) -> tuple[Tensor, Tensor]:
        ...
        # Avoid casting to int64 on CPU. Send as 16-bit to preserve PCIe bandwidth over host-to-device limit, then cast on GPU
        local = chunk[start : start + per_rank_span]
        x = local[:-1].reshape(-1, seq_len)
        y = local[1:].reshape(-1, seq_len)
        return x.to(self.device, non_blocking=True).to(torch.int64), y.to(self.device, non_blocking=True).to(torch.int64)
