...
def quantize_float_tensor(t: Tensor) -> tuple[Tensor, Tensor]:
    ...
    if t32.ndim == 2:
        ...
        compressed_mag = torch.log1p(31.0 * mag) / torch.log1p(torch.tensor(31.0, device=t.device))
        q = torch.clamp(torch.round(torch.sign(clipped) * compressed_mag * 31.0), -31, 31).to(torch.int8).contiguous()
        return q, scale.to(dtype=INT8_PER_ROW_SCALE_DTYPE).contiguous()

    # Vectors / scalars use a simpler per-tensor scale.
    clip_abs = float(torch.quantile(t32.abs().flatten(), INT8_CLIP_Q).item()) if t32.numel() else 0.0
    scale = torch.tensor(clip_abs if clip_abs > 0 else 1.0, dtype=torch.float32, device=t.device)
    clipped = torch.clamp(t32, -clip_abs, clip_abs)
    mag = torch.abs(clipped) / scale
    compressed_mag = torch.log1p(31.0 * mag) / torch.log1p(torch.tensor(31.0, device=t.device))
    q = torch.clamp(torch.round(torch.sign(clipped) * compressed_mag * 31.0), -31, 31).to(torch.int8).contiguous()
    return q, scale
...
def dequantize_state_dict_int8(obj: dict[str, object]) -> dict[str, Tensor]:
    ...
    for name, q in obj["quantized"].items():
        ...
        q_mag = torch.abs(q.float()) / 31.0
        decompressed_mag = (torch.exp(q_mag * torch.log1p(torch.tensor(31.0, device=q.device))) - 1.0) / 31.0
        q_val = torch.sign(q.float()) * decompressed_mag
        
        if qmeta.get(name, {}).get("scheme") == "per_row" or s.ndim > 0:
            s = s.to(dtype=torch.float32)
            # Broadcast the saved row scale back across trailing dimensions.
            out[name] = (q_val * s.view(q.shape[0], *([1] * (q.ndim - 1)))).to(dtype=dtype).contiguous()
        else:
            scale = float(s.item())
            out[name] = (q_val * scale).to(dtype=dtype).contiguous()
