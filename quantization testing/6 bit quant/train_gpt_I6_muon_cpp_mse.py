...
def quantize_float_tensor(t: Tensor) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim >= 2:
        # Dynamic Per-Row MSE Grid Search mapping (64 linear sequences to avoid quantization drift).
        orig_shape = t32.shape
        features = orig_shape[-1]
        t_flat = t32.view(-1, features)
        
        if t_flat.numel() == 0:
            scale_shape = list(orig_shape[:-1])
            return torch.empty(orig_shape, dtype=torch.int8), torch.empty(scale_shape, dtype=INT8_PER_ROW_SCALE_DTYPE)

        # Baseline fast row bounds
        max_abs = torch.max(t_flat.abs(), dim=1, keepdim=True).values

        # Candidate alpha search planes (1, 64)
        alpha_search = torch.linspace(0.4, 1.3, 64, device=t32.device).view(1, 64)
        
        chunk_size = 512 # row boundaries
        best_scales = torch.empty((t_flat.shape[0], 1), dtype=torch.float32, device=t32.device)
        
        for s in range(0, t_flat.shape[0], chunk_size):
            e = min(s + chunk_size, t_flat.shape[0])
            w_chunk = t_flat[s:e] # shape (chunk, features)
            c_chunk = max_abs[s:e] # shape (chunk, 1)
            
            # Broadcast bounds -> (chunk, 64)
            bounds = c_chunk * alpha_search
            cand_scales = (bounds / 31.0).clamp_min(1.0 / 31.0) # (chunk, 64)
            
            # W expanded: (chunk, features, 1), scales: (chunk, 1, 64)
            w_exp = w_chunk.unsqueeze(-1)
            scale_exp = cand_scales.unsqueeze(1)
            q_cands = torch.clamp(torch.round(w_exp / scale_exp), -31, 31)
            recon = q_cands * scale_exp
            
            # MSE
            mse = ((w_exp - recon) ** 2).mean(dim=1) # shape (chunk, 64)
            best_idx = mse.argmin(dim=-1, keepdim=True) # shape (chunk, 1)
            
            # Gather optimal scale directly
            best_scales[s:e] = torch.gather(cand_scales, 1, best_idx)
            
        clipped = torch.maximum(torch.minimum(t_flat, best_scales * 31.0), -best_scales * 31.0)
        q_flat = torch.clamp(torch.round(clipped / best_scales), -31, 31).to(torch.int8)
        
        q = q_flat.reshape(orig_shape).contiguous()
        scale = best_scales.reshape(orig_shape[:-1]).to(dtype=INT8_PER_ROW_SCALE_DTYPE).contiguous()
        return q, scale
