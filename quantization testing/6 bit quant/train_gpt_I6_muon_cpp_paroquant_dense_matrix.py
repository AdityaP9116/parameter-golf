...
_H_512_cpu = torch.tensor([[1.0]], dtype=torch.float32)
while _H_512_cpu.size(0) < 512:
    _H_512_cpu = torch.cat([
        torch.cat([_H_512_cpu, _H_512_cpu], dim=1),
        torch.cat([_H_512_cpu, -_H_512_cpu], dim=1)
    ], dim=0)
_H_512_cpu = _H_512_cpu / math.sqrt(512)

def fwht(x: Tensor) -> Tensor:
    H = _H_512_cpu.to(device=x.device, dtype=x.dtype)
    d = x.shape[-1]
    if d == 512:
        return F.linear(x, H)
    chunks = torch.split(x, 512, dim=-1)
    return torch.cat([F.linear(c, H) for c in chunks], dim=-1)
