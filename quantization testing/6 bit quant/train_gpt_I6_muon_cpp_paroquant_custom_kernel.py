...
__global__ void fwht_kernel(nv_bfloat16* x, int batch_size) {
    int b = blockIdx.x; 
    int tid = threadIdx.x;
    ...
    int base = b * 512;
    smem[tid] = __bfloat162float(x[base + tid]);
    smem[tid + 256] = __bfloat162float(x[base + tid + 256]);
    __syncthreads();
    ...
    float scale = 0.04419417382f; 
    x[base + tid] = __float2bfloat16(smem[tid] * scale);
    x[base + tid + 256] = __float2bfloat16(smem[tid + 256] * scale);
}

void launch_fwht(at::BFloat16* x, int batch_size) {
    fwht_kernel<<<batch_size, 256>>>(reinterpret_cast<nv_bfloat16*>(x), batch_size);
}
"""
...
torch::Tensor fwht_cuda_impl(torch::Tensor X) {
    auto X_bf16 = X.to(torch::kBFloat16).clone().contiguous();
    int batch_size = X_bf16.numel() / 512;
    launch_fwht(X_bf16.data_ptr<at::BFloat16>(), batch_size);
    return X_bf16;
}
...
muon_cuda_module = torch.utils.cpp_extension.load_inline(
    ...
    functions=["zeropower_via_newtonschulz5_cuda_impl", "fwht_cuda_impl"],
    verbose=False,
)
...
def quantize_float_tensor(t: Tensor) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim >= 2:
        # Matrices and Banks get one scale per row, tracking output-channel ranges
        clip_abs = (
        ...
        q = torch.clamp(torch.round(clipped / scale), -31, 31).to(torch.int8).contiguous()
...
def dequantize_state_dict_int8(obj: dict[str, object]) -> dict[str, Tensor]:
    ...
    for name, q in obj["quantized"].items():
        ...
        if qmeta.get(name, {}).get("scheme") == "per_row" or s.ndim > 0:
            ...
            # Broadcast the saved row scale back across trailing dimensions (-1).
...
class STEQuantizeI6(torch.autograd.Function):
    @staticmethod
    def forward(ctx, weight: Tensor) -> Tensor:
        ...
        scale = (clip_abs / 31.0).clamp_min(1.0 / 31.0)
        q = torch.clamp(torch.round(clipped / scale), -31, 31).to(torch.int8)
...
class FWHTFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x: Tensor) -> Tensor:
        d = x.shape[-1]
        if (d & (d - 1)) == 0:
            return muon_cuda_module.fwht_cuda_impl(x).to(x.dtype).view_as(x)
        # 1536 chunking
        chunk_size = 512
        chunks = torch.split(x, chunk_size, dim=-1)
        rotated_chunks = [muon_cuda_module.fwht_cuda_impl(c.contiguous()) for c in chunks]
        return torch.cat(rotated_chunks, dim=-1).to(x.dtype).view_as(x)

    @staticmethod
    def backward(ctx, grad_output: Tensor):
        d = grad_output.shape[-1]
        if (d & (d - 1)) == 0:
            return muon_cuda_module.fwht_cuda_impl(grad_output).to(grad_output.dtype).view_as(grad_output)
        chunk_size = 512
        chunks = torch.split(grad_output, chunk_size, dim=-1)
        rotated_chunks = [muon_cuda_module.fwht_cuda_impl(c.contiguous()) for c in chunks]
        return torch.cat(rotated_chunks, dim=-1).to(grad_output.dtype).view_as(grad_output)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor) -> Tensor:
        ...
        x_rot = fwht(x)
        q = F.linear(x_rot, q_w.to(x.dtype)).reshape(bsz, seqlen, self.num_heads, self.head_dim).transpose(1, 2)
        k = F.linear(x_rot, k_w.to(x.dtype)).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim).transpose(1, 2)
        v = F.linear(x_rot, v_w.to(x.dtype)).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim).transpose(1, 2)
        ...
        y_rot = fwht(y)
        return F.linear(y_rot, out_w.to(x.dtype))
...
class MLP(nn.Module):
    ...
    def forward(self, x: Tensor, up_w: Tensor, down_w: Tensor) -> Tensor:
        x_rot = fwht(x)
        up_out = torch.relu(F.linear(x_rot, up_w.to(x.dtype)))
        up_out_rot = fwht(up_out.square())
        return F.linear(up_out_rot, down_w.to(x.dtype))
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, up_w: Tensor, down_w: Tensor) -> Tensor:
        ...
        attn_out = self.attn(self.attn_norm(x_in), q_w, k_w, v_w, out_w)
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        emb_q = STEQuantizeI6.apply(self.tok_emb.weight)
        x = F.embedding(input_ids, emb_q)
        x = F.rms_norm(x, (x.size(-1),))
        x0 = x
        skips: list[Tensor] = []

        qo = STEQuantizeI6.apply(self.qo_bank)
        kv = STEQuantizeI6.apply(self.kv_bank)
        up = STEQuantizeI6.apply(self.mlp_up_bank)
        down = STEQuantizeI6.apply(self.mlp_down_bank)
        n = self.num_layers
        ...
        for i in range(self.num_decoder_layers):
            ...
            x = self.blocks[idx](x, x0, q_w, k_w, v_w, out_w, up_w, down_w)
        ...
        if self.tie_embeddings:
            ...
            logits_proj = F.linear(x, STEQuantizeI6.apply(self.lm_head.weight))
