...
def quantize_float_tensor(t: Tensor) -> tuple[Tensor, Tensor]:
    ...
    if t32.ndim >= 2:
        ...
        for s in range(0, t_flat.shape[0], chunk_size):
            ...
            q_cands = torch.clamp(torch.round(w_exp / scale_exp), -7, 7)
        ...
        clipped = torch.maximum(torch.minimum(t_flat, best_scales * 7.0), -best_scales * 7.0)
        q_flat = torch.clamp(torch.round(clipped / best_scales), -7, 7).to(torch.int8)
