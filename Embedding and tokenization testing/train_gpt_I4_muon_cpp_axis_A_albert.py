...
class GPT(nn.Module):
    def __init__(
        ...
        self.albert_dim = 64
        self.tok_dict = nn.Embedding(vocab_size, self.albert_dim)
        self.tok_proj = nn.Linear(self.albert_dim, model_dim, bias=False)
        ...
        # Fix: Re-enable full banking for all layers
    ...
    def _init_weights(self) -> None:
        if self.tie_embeddings:
            nn.init.normal_(self.tok_dict.weight, mean=0.0, std=self.tied_embed_init_std)
            nn.init.normal_(self.tok_proj.weight, mean=0.0, std=self.tied_embed_init_std)
        n = self.num_layers
...
def main() -> None:
    ...
    optimizer_tok = torch.optim.Adam(
        [{"params": [base_model.tok_dict.weight, base_model.tok_proj.weight], "lr": token_lr, "base_lr": token_lr}],
