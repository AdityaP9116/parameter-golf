...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, pos_ids: Tensor, cu_seqlens: Tensor, max_seqlen: int) -> Tensor:
        ...
        # Rotary embeddings extracted exactly along position boundaries
        cos, sin = self.rotary(max(1024, max_seqlen), x.device, q.dtype)
        ...
        q = q_rot * self.q_gain.to(dtype=q.dtype)[None, :, None]
        k = k_rot
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, up_w: Tensor, down_w: Tensor, pos_ids: Tensor, cu_seqlens: Tensor, max_seqlen: int) -> Tensor:
        mix = self.resid_mix.to(dtype=x.dtype)
        x_in = mix[0] * x + mix[1] * x0
        attn_out = self.attn(self.attn_norm(x_in), q_w, k_w, v_w, out_w, pos_ids, cu_seqlens, max_seqlen)
...
class GPT(nn.Module):
    def __init__(
        ...
        self.inner_emb_dim = 64
        self.tok_emb_inner = nn.Embedding(vocab_size, self.inner_emb_dim)
        self.tok_emb_proj = nn.Linear(self.inner_emb_dim, model_dim, bias=False)
        ...
        self.lm_head_proj = nn.Linear(model_dim, self.inner_emb_dim, bias=False)
        self.lm_head = nn.Linear(self.inner_emb_dim, vocab_size, bias=False)
    ...
    def tie_weights(self) -> None:
        if self.tie_embeddings:
            self.lm_head.weight = self.tok_emb_inner.weight
        else:
            self.lm_head._zero_init = True

    def _init_weights(self) -> None:
        if self.tie_embeddings:
            nn.init.normal_(self.tok_emb_inner.weight, mean=0.0, std=self.tied_embed_init_std)
        n = self.num_layers
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, pos_ids: Tensor, cu_seqlens: Tensor, max_seqlen: int) -> Tensor:
        emb_w = self.tok_emb_inner.weight
        x_inner = F.embedding(input_ids, emb_w)
        x = self.tok_emb_proj(x_inner)
        ...
        for i in range(self.num_encoder_layers):
            ...
            x = self.blocks[i](x, x0, q_w, k_w, v_w, out_w, up_w, down_w, pos_ids, cu_seqlens, max_seqlen)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            x = self.blocks[idx](x, x0, q_w, k_w, v_w, out_w, up_w, down_w, pos_ids, cu_seqlens, max_seqlen)

        x = self.final_norm(x)
        targets = target_ids
        
        x_inner_out = self.lm_head_proj(x)
        logits_proj = self.lm_head(x_inner_out)
...
def main() -> None:
    ...
    base_model.tok_emb_proj.weight.data = base_model.tok_emb_proj.weight.data.float()
    base_model.lm_head_proj.weight.data = base_model.lm_head_proj.weight.data.float()
    
    if not base_model.tie_embeddings:
    ...
    scalar_params.append(base_model.tok_emb_proj.weight)
    scalar_params.append(base_model.lm_head_proj.weight)
    
    token_lr = args.tied_embed_lr if args.tie_embeddings else args.embed_lr
    optimizer_tok = torch.optim.Adam(
        [{"params": [base_model.tok_emb_inner.weight], "lr": token_lr, "base_lr": token_lr}],
