...
def eval_val(
    ...
    if is_final_eval:
        model.train()
        ttt_opt = torch.optim.AdamW(model.parameters(), lr=5e-5, weight_decay=0.01)
    else:
        model.eval()

    import contextlib
    context = contextlib.nullcontext() if is_final_eval else torch.inference_mode()
...
def load_data_shard(file: Path) -> Tensor:
    ...
    tokens_np = np.memmap(file, dtype="<i2", mode="r", offset=header_bytes, shape=(num_tokens,))
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, pos_x: Tensor, cu_seqlens: Tensor, max_seqlen: int, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor) -> Tensor:
        ...
        try:
            ...
            raise RuntimeError("Axis A varlen sequence packing strictly requires flash-attn to run securely without math fragmentation!")
...
class Block(nn.Module):
    def __init__(
        ...
        super().__init__()
        # Parallel Residuals strictly deduplicated LayerNorm parameters
        self.ln = RMSNorm()
    ...
    def forward(self, x: Tensor, x0: Tensor, pos_x: Tensor, cu_seqlens: Tensor, max_seqlen: int, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, up_w: Tensor, down_w: Tensor) -> Tensor:
        ...
        mlp_out = self.mlp(z, up_w, down_w)
...
class GPT(nn.Module):
    def __init__(
        ...
        self.hash_buckets = int(os.environ.get("HASH_BUCKETS", 4096))
        self.hash_emb = nn.Embedding(self.hash_buckets, model_dim)
    ...
    def _init_weights(self) -> None:
        if self.tie_embeddings:
            nn.init.normal_(self.hash_emb.weight, mean=0.0, std=self.tied_embed_init_std)
        n = self.num_layers
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, pos_x: Tensor, cu_seqlens: Tensor, max_seqlen: int) -> Tensor:
        emb_w = self.hash_emb.weight
        hashed_input_ids = input_ids % self.hash_buckets
        x = F.embedding(hashed_input_ids, emb_w)
        ...
        for i in range(self.num_encoder_layers):
            ...
            x = self.blocks[i](x, x0, pos_x, cu_seqlens, max_seqlen, q_w, k_w, v_w, out_w, up_w, down_w)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            x = self.blocks[idx](x, x0, pos_x, cu_seqlens, max_seqlen, q_w, k_w, v_w, out_w, up_w, down_w)

        x = self.final_norm(x)
        targets = target_ids
        if self.tie_embeddings:
            # Create indices for the full vocab and hash them to find their shared bucket
            vocab_indices = torch.arange(self.vocab_size, device=x.device)
            hashed_indices = vocab_indices % self.hash_buckets
            
            # Gather the shared weights to create a virtual [4096, model_dim] head
            tied_lm_weight = self.hash_emb.weight[hashed_indices]
            
            # Project hidden states to full vocab logits
            logits_proj = F.linear(x, tied_lm_weight)
        else:
...
def main() -> None:
    ...
    optimizer_tok = torch.optim.Adam(
        [{"params": [base_model.hash_emb.weight], "lr": token_lr, "base_lr": token_lr}],
